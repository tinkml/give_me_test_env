...
async def test_take_marks_free_stand_occupied_by_caller() -> None:
    repository = FakeStandRepository([Stand(name="slplay7", status="free")])
    command = TakeCommand(repository, StandResolutionPolicy())

    result = await command.execute(user_name="alice", argument="slplay7")
...
async def test_take_unknown_stand_returns_error_message_without_state_change() -> None:
    repository = FakeStandRepository([Stand(name="slplay7", status="free")])
    command = TakeCommand(repository, StandResolutionPolicy())

    result = await command.execute(user_name="alice", argument="unknown")
